...
# Step 1: Load the Enh similarity network
def load_Enh_network(Enh_sim_file="../Data/EnhNetG.txt"):
    print(Enh_sim_file)
    
    print("Loading Enh similarity network...")
    Enh_sim_df = pd.read_csv(Enh_sim_file, sep='\t', header=None, names=['Enh1', 'sim', 'Enh2'])
    Enhs = set(Enh_sim_df['Enh1']).union(set(Enh_sim_df['Enh2']))
    
    Enh_G = nx.Graph()
    for Enh in tqdm(Enhs, desc="Adding Enh nodes"):
        Enh_G.add_node(Enh, type="Enh")
    
    for _, row in tqdm(Enh_sim_df.iterrows(), total=len(Enh_sim_df), desc="Adding Enh-Enh edges"):
        Enh_G.add_edge(row['Enh1'], row['Enh2'], weight=float(row['sim']))
    
    return Enh_G, list(Enhs)
...
def save_embeddings_Enh(Enh_embeddings, Enh_G, output_file):
    print("Saving embeddings...")
    data = []
    
    for node, emb in Enh_embeddings.items():
        node_type = Enh_G.nodes[node]["type"]
        row = [node, node_type] + emb.tolist()
        data.append(row)
    
    embedding_size = len(next(iter(Enh_embeddings.values())))
    columns = ['node_id', 'type'] + [f'dim_{i+1}' for i in range(embedding_size)]
    df = pd.DataFrame(data, columns=columns)
    df.to_csv(output_file, index=False)
    print(f"Enh Embeddings saved to {output_file}")
...
# Main function
def main(embedding_size=128, epochs=50, Enh_sim_file="../Data/EnhNetG.txt"):
    # Load Enh network (only once)
    Enh_base_name = os.path.splitext(os.path.basename(Enh_sim_file))[0]
    Enh_output_file = f"../Results/{Enh_base_name}_{emb_method}_d_{embedding_size}_e_{epochs}.csv" 
    if os.path.exists(Enh_output_file):
        print(f"File {Enh_output_file} is existing!")
    else: 
        Enh_G, Enhs = load_Enh_network(Enh_sim_file)
        print("Converting Enh graph to PyTorch Geometric format...")
        Enh_data, Enh_node_to_idx = graph_to_pyg_data(Enh_G, embedding_size)
        print("Computing Enh embeddings...")
        Enh_embeddings = train_gat(Enh_data, embedding_size=embedding_size, epochs=epochs, node_type="Enhs")
        Enh_embeddings_dict = extract_embeddings(Enh_embeddings, Enhs, Enh_node_to_idx, "Enhs")
        # Save Enh embeddings
        save_embeddings_Enh(Enh_embeddings_dict, Enh_G, Enh_output_file)

        # Print sample embeddings
        print(f"\nSample Enh embeddings for {Enh_output_file}:")
        for node, emb in list(Enh_embeddings_dict.items())[:5]:
            print(f"Enh Node: {node}, Embedding: {emb[:5]}...")
